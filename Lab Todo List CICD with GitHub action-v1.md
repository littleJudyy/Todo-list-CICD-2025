...
```
<img width="1440" height="900" alt="image" src="https://github.com/user-attachments/assets/23958dd2-d358-44a3-bdd5-ed36544ed53e" />
...
# ตอบคำถามที่นี่
1. Health Check
ทดสอบฟังก์ชัน health_check() โดยตรวจว่าเชื่อมต่อฐานข้อมูลได้จริงหรือไม่
กรณีปกติจะตอบกลับสถานะ 200 และขึ้นว่า connected
กรณีฐานข้อมูลล่มจะตอบกลับสถานะ 503 พร้อมข้อความ error

2. Create Todo
ทดสอบฟังก์ชัน create_todo() สำหรับการเพิ่มข้อมูลใหม่
กรณีมีข้อมูลครบจะสร้างได้สำเร็จและตอบ 201
ถ้าไม่กรอก title จะตอบ 400
หากบันทึกลงฐานข้อมูลไม่สำเร็จจะ rollback และตอบ 500

3. Update Todo
ทดสอบฟังก์ชัน update_todo() ในการแก้ไขข้อมูล
กรณีปกติแก้ไขได้สำเร็จและตอบ 200
หากไม่พบ id ที่ต้องการแก้ไขจะตอบ 404
หากเกิดข้อผิดพลาดในการบันทึกจะ rollback และตอบ 500

4. Delete Todo
ทดสอบฟังก์ชัน delete_todo() สำหรับลบข้อมูล
กรณีลบได้สำเร็จจะตอบ 200
ถ้าไม่พบข้อมูลจะตอบ 404
กรณีเกิดข้อผิดพลาดในการลบจะตอบ 500

5. Error Handler
ทดสอบการทำงานของตัวจัดการข้อผิดพลาด (@app.errorhandler)
กรณีเกิดข้อผิดพลาดทั่วไปจะตอบเป็น JSON มาตรฐานและไม่ทำให้ระบบล่ม
กรณีเรียกเส้นทางที่ไม่มีอยู่จะตอบ 404 พร้อมข้อความ Resource not found
